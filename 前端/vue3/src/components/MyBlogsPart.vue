<template>
    <div id="MyBlogsPart" :class="articles[0]?.id ? 'dynamicPadding' : ''">
        ...
        <div class="block" v-for="article in articles" :key="article.id">
            <OneBlog :article="article" :march-text="marchText.trim()" :sort-flag="sortFlag" :sort-name="sortName"
                ...
                :myBlogUserId="userProfile.userId" />
        </div>
        <div class="pagingSelect" v-if="articles[0]">
            ...
            <div class="pagingSetting" @click="clickPagingSetting" ref="pagingSetting" @mouseenter="handleMouseEnter"
                @mouseleave="handleMouseLeave">
                <div class="setting" v-if="ifRenderSetting" @pointerdown.stop @mousedown.stop @click.stop @wheel.stop>
                    <div class="option">跳转至第<input type="text" v-model="directNum">
                        <div><button class="topArrow" @click="changeDirectNum(1)"></button><button class="bottomArrow"
                                @click="changeDirectNum(-1)"></button></div>
                        页<button class="ok" @click="directOK">确认</button>
                    </div>
                    <div class="option">每页展示<input type="text" v-model="displayNum">
                        <div><button class="topArrow" @click="changeDisplayNum(1)"></button><button class="bottomArrow"
                                @click="changeDisplayNum(-1)"></button></div>
...
<script setup lang="ts" name="FavoritePart">
    import { ref, onUpdated, onMounted, watch, toRefs, toRef } from 'vue'
    import { type BlogWithUserName, type UserProfile, type Paging } from '@/types'
    ...
    let { userSpaceSortJS, userSpacePagingJS } = storeToRefs(useUserDataStore())

    let myblogsPagingJS = toRef(userSpacePagingJS.value, 'myblogs');

    //以下是分页对象、分页功能
    let paging = ref<Paging>({
        pageNum: myblogsPagingJS.value.pageNum,
        pageSize: myblogsPagingJS.value.pageSize,
        totalPage: 0,
        totalCount: 0,
        data: []
    });
    ...
    // 以下是展示所有自己的文章的功能
    const { add: addFn, delete_: deleteFn, deleteByArticleId: deleteByArticleIdFn, getDTO: getDTOFn, findByUserId: findByUserIdFn } = useFavoriteApi()

    let articles = ref<BlogWithUserName[]>([{
        id: '',
        title: '',
        authorName: '',
        content: '',
    ...
    async function getBlogs() {
        ...
        myblogsPagingJS.value.pageNum = pageNum.value = paging.value.pageNum;
        myblogsPagingJS.value.pageSize = pageSize.value = paging.value.pageSize;
    ...
    let sortName = ref(userSpaceSortJS.value.sortName)

    let sortFlag = ref(userSpaceSortJS.value.sortFlag)

    watch([sortName, sortFlag], async (value) => {
        userSpaceSortJS.value.sortName = value[0];
        userSpaceSortJS.value.sortFlag = value[1];
        articles.value = await getBlogs();
    })
    ...
    watch(marchText, (value) => {
        userSpaceSortJS.value.marchText = value;
    })

    async function changeToSortUp() {
        sortFlag.value = "up";
        userSpaceSortJS.value.sortFlag = "up"
        articles.value = await getBlogs()
    }

    async function changeToSortDown() {
        sortFlag.value = "down";
        userSpaceSortJS.value.sortFlag = "down"
        articles.value = await getBlogs()
    }
