<template>
    <div id="FavoritePart" :class="articles[0]?.id ? 'dynamicPadding' : ''">
        ...
        <div class="block" v-for="article in articles" :key="article.id">
            <OneBlog :article="article" :march-text="marchText.trim()" :sort-flag="sortFlag" :sort-name="sortName"
                ...
                :favoriteUserId="userProfile.userId" />
        </div>
...
<script setup lang="ts" name="FavoritePart">
    ...
    import { type Blog, type User, type Paging } from '@/types'
    ...
    let favoritePagingJS = toRef(userSpacePagingJS.value, 'favorite');
    ...
    let paging = ref<Paging>({
        pageNum: favoritePagingJS.value.pageNum,
        pageSize: favoritePagingJS.value.pageSize,
        totalPage: 0,
        totalCount: 0,
        data: []
    });
    ...
    async function getBlogs() {
        ...
        favoritePagingJS.value.pageNum = pageNum.value = paging.value.pageNum;
        favoritePagingJS.value.pageSize = pageSize.value = paging.value.pageSize;
